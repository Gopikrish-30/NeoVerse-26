...
pnpm -F @navigator/web test                    # Run all web Vitest tests
pnpm -F @navigator/web test:unit               # Web unit tests only
pnpm -F @navigator/web test:integration        # Web integration tests only

# Testing (desktop workspace — main process + preload tests)
pnpm -F @navigator/desktop test                # Run all desktop Vitest tests
pnpm -F @navigator/desktop test:unit           # Desktop unit tests only
pnpm -F @navigator/desktop test:integration    # Desktop integration tests only
pnpm -F @navigator/desktop test:e2e            # Docker-based E2E tests
pnpm -F @navigator/desktop test:e2e:native     # Native Playwright E2E tests (serial, Electron requirement)

# Testing (agent-core)
pnpm -F @navigator_ai/agent-core test          # Run agent-core Vitest tests
...
pnpm lint && pnpm format:check && pnpm -F @navigator/web test && pnpm -F @navigator/desktop test && pnpm -F @navigator_ai/agent-core test
```
...
- **Do NOT add root-level test scripts** — tests are workspace-scoped (`-F @navigator/web`, `-F @navigator/desktop`, or `-F @navigator_ai/agent-core`)
...
- `@navigator_ai/agent-core` — Core business logic, types, storage, MCP tools (ESM, internal workspace package)
- `@navigator/web` — Standalone React UI (Vite + React Router + Zustand)
- `@navigator/desktop` — Thin Electron shell (main process + preload), loads web's build output

## Code Conventions

- TypeScript everywhere (no JS for app logic)
- **ESM package**: `@navigator_ai/agent-core` uses `"type": "module"` — all imports MUST use `.js` extensions
...
- IPC handlers in `apps/desktop/src/main/ipc/handlers.ts` must match `window.navigatorApp` API in preload
...
3. Add the typed wrapper in `apps/web/src/client/lib/navigatorApp.ts`
...
4. Run `pnpm -F @navigator_ai/agent-core test`
...
```typescript
"@/*"                              → "src/client/*"
"@navigator_ai/agent-core"        → "../../packages/agent-core/src/index.ts"
"@navigator_ai/agent-core/*"      → "../../packages/agent-core/src/*"
"@navigator_ai/agent-core/common" → "../../packages/agent-core/src/common.ts"
```
...
- `NAVIGATOR_BUNDLED_MCP=1` - Bundle MCP tools in packaged build (used in package/release scripts)
