...
describe('connection', () => {
  beforeEach(() => {
    ...
    delete process.env.NAVIGATOR_TASK_ID;
  });
  ...
  describe('configureFromEnv', () => {
    ...
    it('uses NAVIGATOR_TASK_ID for task isolation', () => {
      process.env.NAVIGATOR_TASK_ID = 'task-abc';
      const cfg = configureFromEnv();
      expect(cfg.taskId).toBe('task-abc');
    });

    it('defaults taskId to "default" when NAVIGATOR_TASK_ID is not set', () => {
      const cfg = configureFromEnv();
  ...
  describe('getFullPageName', () => {
    ...
    it('uses configureFromEnv taskId', () => {
      process.env.NAVIGATOR_TASK_ID = 'env-task';
      configureFromEnv();
