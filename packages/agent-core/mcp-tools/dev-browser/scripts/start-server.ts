...
function getDataDir(): string {
  ...
  if (process.platform === 'darwin') {
    return join(homeDir, 'Library', 'Application Support', 'Navigator', 'dev-browser');
  } else if (process.platform === 'win32') {
    return join(process.env.APPDATA || homeDir, 'Navigator', 'dev-browser');
  } else {
    return join(homeDir, '.navigator', 'dev-browser');
  }
}
...
const NAVIGATOR_HTTP_PORT = parseInt(process.env.DEV_BROWSER_PORT || '9224', 10);
const NAVIGATOR_CDP_PORT = parseInt(process.env.DEV_BROWSER_CDP_PORT || '9225', 10);

if (
  !Number.isFinite(NAVIGATOR_HTTP_PORT) ||
  NAVIGATOR_HTTP_PORT < 1 ||
  NAVIGATOR_HTTP_PORT > 65535
) {
  throw new Error(
...
try {
  const res = await fetch(`http://localhost:${NAVIGATOR_HTTP_PORT}`, {
    ...
    if (info.mode === 'extension') {
      ...
      try {
        if (process.platform === 'win32') {
          const output = execSync(`netstat -ano | findstr :${NAVIGATOR_HTTP_PORT}`, {
            encoding: 'utf-8',
          });
          ...
          const pid = execSync(`lsof -ti:${NAVIGATOR_HTTP_PORT}`, { encoding: 'utf-8' }).trim();
          if (pid) {
            execSync(`kill -9 ${pid}`);
          }
        }
      ...
      console.log(`Launch server already running on port ${NAVIGATOR_HTTP_PORT}`);
      process.exit(0);
    }
  }
} catch {
...
try {
  if (process.platform === 'win32') {
    const output = execSync(`netstat -ano | findstr :${NAVIGATOR_CDP_PORT}`, {
      encoding: 'utf-8',
    });
    const match = output.match(/LISTENING\s+(\d+)/);
    if (match) {
      const pid = match[1];
      console.log(
        `Cleaning up stale Chrome process on CDP port ${NAVIGATOR_CDP_PORT} (PID: ${pid})`,
      );
      execSync(`taskkill /F /PID ${pid}`, { stdio: 'ignore' });
    }
  } else {
    const pid = execSync(`lsof -ti:${NAVIGATOR_CDP_PORT}`, { encoding: 'utf-8' }).trim();
    if (pid) {
      console.log(
        `Cleaning up stale Chrome process on CDP port ${NAVIGATOR_CDP_PORT} (PID: ${pid})`,
      );
...
async function startServer(retry = false): Promise<void> {
  try {
    const server = await serve({
      port: NAVIGATOR_HTTP_PORT,
      cdpPort: NAVIGATOR_CDP_PORT,
      headless,
      profileDir,
