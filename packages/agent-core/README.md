# @navigator_ai/agent-core

Core library for the Navigator desktop automation assistant. Provides factory-based APIs for task management, persistent storage, permission handling, logging, and more.

## Usage

This package is internal to the Navigator monorepo and is consumed via workspace dependencies (for example, `"@navigator_ai/agent-core": "workspace:*"`).

## Quick Start

```typescript
import { createStorage, createTaskManager } from '@navigator_ai/agent-core';

// Initialize storage (SQLite + encrypted secure storage)
const storage = createStorage({
  databasePath: '/path/to/navigator.db',
...
```typescript
import { createTaskManager } from '@navigator_ai/agent-core';
```
...
```typescript
import { createStorage } from '@navigator_ai/agent-core';
```
...
```typescript
import { createPermissionHandler } from '@navigator_ai/agent-core';
```
...
```typescript
import { createThoughtStreamHandler } from '@navigator_ai/agent-core';
```
...
```typescript
import { createLogWriter } from '@navigator_ai/agent-core';
```
...
```typescript
import { createSkillsManager } from '@navigator_ai/agent-core';
```
...
```typescript
import { createSpeechService } from '@navigator_ai/agent-core';
```
...
```typescript
import { generateConfig, type BrowserConfig } from '@navigator_ai/agent-core';
