...
export class OpenCodeAdapter extends EventEmitter<OpenCodeAdapterEvents> {
  ...
  async interruptTask(): Promise<void> {
    ...
    // Fallback: force-kill if the process doesn't exit within 5 seconds
    const ptyRef = this.ptyProcess;
    setTimeout(() => {
      if (this.ptyProcess === ptyRef && this.ptyProcess) {
        console.log('[OpenCode CLI] Process did not exit after interrupt, force-killing');
        try {
          this.ptyProcess.kill();
        } catch (err) {
          console.warn('[OpenCode CLI] Error force-killing after interrupt:', err);
        }
        this.ptyProcess = null;
      }
    }, 5000);
