...
export async function buildProviderConfigs(
  ...
  if (
    ...
    providerConfigs.push({
      ...
      models: {
        // Conservative output token cap to avoid provider 402 (insufficient budget)
        [modelId]: { name: modelId, tools: true, limit: { output: 4096 } },
      },
    });
  ...
  if (groqProvider?.connectionStatus === 'connected' && groqProvider.selectedModelId) {
    ...
    console.log('[OpenCode Config Builder] Groq configured:', modelId);
  } else {
    const groqKey = getApiKey('groq');
    if (groqKey) {
      const selectedModel = getSelectedModel();
      if (selectedModel?.provider === 'groq' && selectedModel.model) {
        const modelId = selectedModel.model.replace(/^groq\//, '');
        providerConfigs.push({
          id: 'groq',
          npm: '@ai-sdk/openai-compatible',
          name: 'Groq',
        ...
        console.log('[OpenCode Config Builder] Groq (legacy) configured:', modelId);
      }
    }
  }
