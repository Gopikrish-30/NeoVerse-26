...
if (!url) {
  ...
  console.error(
    'Example: pnpm dev:remote https://navigatorApp-app-preview-42.navigatorApp.workers.dev',
  );
  process.exit(1);
}
...
const env = {
  ...process.env,
  NAVIGATOR_ROUTER_URL: url,
};

console.log('[dev:remote] Launching Electron → ' + url);

const electronArgs = ['-F', '@navigator/desktop', 'dev:remote'];
if (isCheck) {
