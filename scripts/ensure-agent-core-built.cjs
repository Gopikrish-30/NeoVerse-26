...
if (hostNodeTarget) {
  if (!isHostNodeRuntimeAvailable(hostNodeTarget)) {
    ...
    runPnpm(
      ['-F', '@navigator/desktop', 'download:nodejs', `--platform=${hostNodeTarget}`],
...
if (needsAgentCoreBuild) {
  console.log('Missing @navigator_ai/agent-core build outputs:');
  ...
  runPnpm(['-F', '@navigator_ai/agent-core', 'build'], 'Building @navigator_ai/agent-core...');

  missingOutputs = [...outputPaths].filter(
    (relativeOutputPath) => !fs.existsSync(path.join(agentCoreDir, relativeOutputPath)),
  );
  if (missingOutputs.length > 0) {
    console.error('Failed to produce required @navigator_ai/agent-core outputs:');
    for (const missingOutput of missingOutputs) {
...
if (missingMcpDistOutputs.length > 0) {
  ...
  runPnpm(
    ['-F', '@navigator/desktop', 'build:mcp-tools:dev'],
...
console.log('✓ @navigator_ai/agent-core and MCP dist outputs found');
process.exit(0);
