...
import { hasAnyReadyProvider, isProviderReady } from '@navigator_ai/agent-core/common';
...
import { Key, Lightning, Microphone, Info, Plugs, PaperPlaneTilt } from '@phosphor-icons/react';
...
const TABS = [
  ...
  { id: 'telegram' as const, labelKey: 'tabs.telegram', icon: PaperPlaneTilt },
...
interface SettingsDialogProps {
   ...
   */
  initialTab?: 'providers' | 'voice' | 'skills' | 'connectors' | 'telegram' | 'about';
}
...
export function SettingsDialog({
  ...
  const [activeTab, setActiveTab] = useState<
    'providers' | 'voice' | 'skills' | 'connectors' | 'telegram' | 'about'
  >(initialTab);
  ...
  useEffect(() => {
    ...
    navigatorApp.getDebugMode().then(setDebugModeState);
    // Load app version
    navigatorApp.getVersion().then(setAppVersion);
  ...
  const handleDebugToggle = useCallback(async () => {
    ...
    await navigatorApp.setDebugMode(newValue);
    setDebugModeState(newValue);
  }, [debugMode, navigatorApp]);
      ...
      >
        ...
        <nav className="w-48 shrink-0 border-r border-border bg-muted/30 p-3 flex flex-col gap-1">
          <div className="px-3 py-2 mb-1">
            <span className="text-sm font-semibold text-foreground pl-1.5">Navigator</span>
          </div>
          {TABS.map((tab) => (
            <button
              key={tab.id}
