...
export function useSpeechInput(options: UseSpeechInputOptions = {}): UseSpeechInputState & {
  ...
  useEffect(() => {
    navigatorApp.speechIsConfigured().then((configured) => {
  ...
  const stopRecording = useCallback(async () => {
    ...
    try {
      ...
      const result = await navigatorApp.speechTranscribe(audioData, 'audio/webm');
  ...
  const retry = useCallback(async () => {
    ...
    try {
      ...
      const result = await navigatorApp.speechTranscribe(lastAudioDataRef.current, 'audio/webm');
  ...
  /**
   * Re-check if speech input is configured (call after settings change)
   */
  const recheckConfiguration = useCallback(async () => {
    const configured = await navigatorApp.speechIsConfigured();
    setState((prev) => ({ ...prev, isConfigured: configured }));
  }, [navigatorApp]);
