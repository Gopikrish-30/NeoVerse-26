/**
 * Navigator API - Interface to the Electron main process
 *
 * This module provides type-safe access to the navigatorApp API
...
interface NavigatorAppAPI {
  ...
  telegramRemoveToken(): Promise<void>;
  telegramSetEnabled(enabled: boolean): Promise<void>;
  telegramGetStatus(): Promise<{
    enabled: boolean;
    running: boolean;
    paired: boolean;
    botUsername?: string;
  ...
  telegramGeneratePairing(): Promise<{
    pin: string;
    qrCodeDataUrl: string;
    botLink: string;
  }>;
  telegramUnpair(): Promise<void>;
  telegramHasToken(): Promise<boolean>;
  telegramApprovePairing(requestId: string): Promise<void>;
  telegramDenyPairing(requestId: string): Promise<void>;
  onTelegramPairingRequest?(
    callback: (request: {
      id: string;
      chatId: number;
      userId: number;
...
declare global {
  interface Window {
    navigatorApp?: NavigatorAppAPI;
    navigatorAppShell?: NavigatorAppShell;
  }
}

/**
 * Get the navigatorApp API
 * Throws if not running in Electron
 */
export function getNavigatorApp() {
  if (!window.navigatorApp) {
    throw new Error('Navigator API not available - not running in Electron');
  }
  return {
    ...window.navigatorApp,
    ...
    validateBedrockCredentials: async (
      ...
      return window.navigatorApp!.validateBedrockCredentials(JSON.stringify(credentials));
    },

    saveBedrockCredentials: async (credentials: BedrockCredentials): Promise<ApiKeyConfig> => {
      return window.navigatorApp!.saveBedrockCredentials(JSON.stringify(credentials));
    },

    getBedrockCredentials: async (): Promise<BedrockCredentials | null> => {
      return window.navigatorApp!.getBedrockCredentials();
    },

    fetchBedrockModels: (credentials: string) =>
      window.navigatorApp!.fetchBedrockModels(credentials),

    validateVertexCredentials: async (
      credentials: VertexCredentials,
    ): Promise<{ valid: boolean; error?: string }> => {
      return window.navigatorApp!.validateVertexCredentials(JSON.stringify(credentials));
    },

    saveVertexCredentials: async (credentials: VertexCredentials): Promise<ApiKeyConfig> => {
      return window.navigatorApp!.saveVertexCredentials(JSON.stringify(credentials));
    },

    getVertexCredentials: async (): Promise<VertexCredentials | null> => {
      return window.navigatorApp!.getVertexCredentials();
    },

    fetchVertexModels: (credentials: string) => window.navigatorApp!.fetchVertexModels(credentials),

    detectVertexProject: () => window.navigatorApp!.detectVertexProject(),

    listVertexProjects: () => window.navigatorApp!.listVertexProjects(),
  };
}
...
export function isRunningInElectron(): boolean {
  return window.navigatorAppShell?.isElectron === true;
}
...
export function getShellVersion(): string | null {
  return window.navigatorAppShell?.version ?? null;
}
...
export function getShellPlatform(): string | null {
  return window.navigatorAppShell?.platform ?? null;
}

/**
 * React hook to use the navigatorApp API
 */
export function useNavigatorApp(): NavigatorAppAPI {
  const api = window.navigatorApp;
  if (!api) {
    throw new Error('Navigator API not available - not running in Electron');
  }
  return api;
