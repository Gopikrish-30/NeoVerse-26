  ...
  private createTelegramCallbacks(taskId: string, chatId: number) {
    ...
    return {
      onBatchedMessages: (messages: TaskMessage[]) => {
        ...
        if (formatted) {
          void this.bot.sendHtml(chatId, formatted).catch((err) => {
      ...
      onProgress: (progress: { stage: string; message?: string }) => {
        ...
        void this.bot?.sendHtml(chatId, text).catch((err) => {
      ...
      onPermissionRequest: (request: unknown) => {
        ...
        void this.bot?.sendPermissionRequest(chatId, text, req.requestId).catch((err) => {
