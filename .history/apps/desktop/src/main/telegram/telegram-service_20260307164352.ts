...
export class TelegramService {
  ...
  async setBotToken(token: string): Promise<{ username: string; firstName: string }> {
    ...
    const tempBot = new TelegramBot(token, this.bridge, null, () => {});
    const info = await tempBot.getBotInfo();
    if (!info) {
      throw new Error('Invalid bot token. Could not connect to Telegram API.');
    }
    await tempBot.stop();

    // Store token encrypted
    const storage = getStorage();
    storage.storeApiKey(TELEGRAM_TOKEN_STORAGE_KEY, token);
    this.botToken = token;
