...
export function formatTaskStarted(taskId: string, prompt: string): string {
  ...
  return (
    `🔄 <b>Task Started</b> <code>${shortId}</code>\n\n` + `📋 ${escapeHtml(truncate(prompt, 500))}`
  );
}
...
export function formatTaskComplete(taskId: string, result: TaskResult): string {
  ...
  let text = `${icon} <b>Task ${statusText}</b> <code>${shortId}</code>`;

  if (result.summary) {
    text += `\n\n📝 <b>Summary:</b>\n${escapeHtml(truncate(result.summary, 1000))}`;
  }

  if (result.error) {
    text += `\n\n⚠️ <b>Error:</b> ${escapeHtml(truncate(result.error, 500))}`;
  }
...
export function formatTaskError(taskId: string, error: string): string {
  const shortId = taskId.slice(0, 8);
  return (
    `❌ <b>Task Failed</b> <code>${shortId}</code>\n\n` + `⚠️ ${escapeHtml(truncate(error, 500))}`
  );
}
