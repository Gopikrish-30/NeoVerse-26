...
if (!gotTheLock) {
  ...
  app.whenReady().then(async () => {
    ...
    if (process.env.ENABLE_MEMORY_LOGGING === '1') {
      try {
        const collector = getLogCollector();
        __memoryLogInterval = setInterval(async () => {
          try {
            const mainMem = await process.getProcessMemoryInfo();
            collector.log('INFO', 'memory', 'mainProcessMemory', mainMem);
            if (mainWindow && !mainWindow.isDestroyed()) {
              const rendererMem = await mainWindow.webContents.getProcessMemoryInfo();
              collector.log('INFO', 'memory', 'rendererProcessMemory', rendererMem);
            }
          } catch (err) {
