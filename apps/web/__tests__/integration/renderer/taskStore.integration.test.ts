 ...
 * Tests store actions with mocked window.navigatorApp API
...
// Mock navigatorApp API
const mockNavigatorApp = {
  startTask: vi.fn(),
  cancelTask: vi.fn(),
  interruptTask: vi.fn(),
  resumeSession: vi.fn(),
...
// Mock the navigatorApp module
vi.mock('@/lib/navigator-app', () => ({
  getNavigatorApp: () => mockNavigatorApp,
}));

// Mock window.navigatorApp for global subscriptions
...
describe('taskStore Integration', () => {
  ...
  describe('startTask', () => {
    it('should call startTask API and update state on success', async () => {
      ...
      mockNavigatorApp.startTask.mockResolvedValueOnce(mockTask);
      ...
      // Assert
      expect(mockNavigatorApp.startTask).toHaveBeenCalledWith(config);
    ...
    it('should set error state on failure', async () => {
      // Arrange
      const { useTaskStore } = await import('@/stores/taskStore');
      mockNavigatorApp.startTask.mockRejectedValueOnce(new Error('API Error'));
    ...
    it('should handle non-Error exceptions gracefully', async () => {
      // Arrange
      const { useTaskStore } = await import('@/stores/taskStore');
      mockNavigatorApp.startTask.mockRejectedValueOnce('String error');
    ...
    it('should update existing task if same ID', async () => {
      ...
      mockNavigatorApp.startTask.mockResolvedValueOnce(updatedTask);
  ...
  describe('sendFollowUp', () => {
    ...
    it('should start fresh task for interrupted task without session', async () => {
      ...
      mockNavigatorApp.startTask.mockResolvedValueOnce(newTask);
      ...
      // Assert
      expect(mockNavigatorApp.startTask).toHaveBeenCalled();
    });
    ...
    it('should resume session when task has sessionId', async () => {
      ...
      mockNavigatorApp.resumeSession.mockResolvedValueOnce(resumedTask);
      ...
      // Assert
      expect(mockNavigatorApp.resumeSession).toHaveBeenCalledWith(
        'session-abc',
        'Continue please',
        'task-123',
      );
    ...
    it('should add user message optimistically', async () => {
      ...
      mockNavigatorApp.resumeSession.mockResolvedValueOnce(
    ...
    it('should handle resumeSession failure', async () => {
      ...
      mockNavigatorApp.resumeSession.mockRejectedValueOnce(new Error('Resume failed'));
  ...
  describe('cancelTask', () => {
    it('should call cancelTask API and update status', async () => {
      ...
      mockNavigatorApp.cancelTask.mockResolvedValueOnce(undefined);
      ...
      // Assert
      expect(mockNavigatorApp.cancelTask).toHaveBeenCalledWith('task-123');
    ...
    it('should do nothing when no current task', async () => {
      ...
      // Assert
      expect(mockNavigatorApp.cancelTask).not.toHaveBeenCalled();
    });
  });
  ...
  describe('interruptTask', () => {
    it('should call interruptTask API for running task', async () => {
      ...
      mockNavigatorApp.interruptTask.mockResolvedValueOnce(undefined);

      // Act
      await useTaskStore.getState().interruptTask();

      // Assert
      expect(mockNavigatorApp.interruptTask).toHaveBeenCalledWith('task-123');
    });
    ...
    it('should not call API for non-running task', async () => {
      ...
      // Assert
      expect(mockNavigatorApp.interruptTask).not.toHaveBeenCalled();
    });
  ...
  describe('error state management', () => {
    it('should clear error on successful task start', async () => {
      ...
      mockNavigatorApp.startTask.mockResolvedValueOnce(createMockTask('task-123'));
  ...
  describe('loadTasks', () => {
    it('should load tasks from API', async () => {
      ...
      mockNavigatorApp.listTasks.mockResolvedValueOnce(mockTasks);
      ...
      // Assert
      expect(mockNavigatorApp.listTasks).toHaveBeenCalled();
  ...
  describe('loadTaskById', () => {
    it('should load specific task and set as current', async () => {
      ...
      mockNavigatorApp.getTask.mockResolvedValueOnce(mockTask);
      ...
      // Assert
      expect(mockNavigatorApp.getTask).toHaveBeenCalledWith('task-123');
      expect(state.currentTask).toEqual(mockTask);
      expect(state.error).toBeNull();
    });
    ...
    it('should set error when task not found', async () => {
      // Arrange
      const { useTaskStore } = await import('@/stores/taskStore');
      mockNavigatorApp.getTask.mockResolvedValueOnce(null);
  ...
  describe('deleteTask', () => {
    it('should delete task and remove from list', async () => {
      ...
      mockNavigatorApp.deleteTask.mockResolvedValueOnce(undefined);
      ...
      // Assert
      expect(mockNavigatorApp.deleteTask).toHaveBeenCalledWith('task-2');
      expect(state.tasks).toHaveLength(2);
  ...
  describe('clearHistory', () => {
    it('should clear all tasks', async () => {
      ...
      mockNavigatorApp.clearTaskHistory.mockResolvedValueOnce(undefined);
      ...
      // Assert
      expect(mockNavigatorApp.clearTaskHistory).toHaveBeenCalled();
      expect(state.tasks).toEqual([]);
    });
  });
  ...
  describe('respondToPermission', () => {
    it('should call API and clear permission request', async () => {
      ...
      mockNavigatorApp.respondToPermission.mockResolvedValueOnce(undefined);
      ...
      // Assert
      expect(mockNavigatorApp.respondToPermission).toHaveBeenCalledWith(response);
      expect(state.permissionRequest).toBeNull();
    });
  });
