...
import type { Task, TaskStatus, TaskMessage, PermissionRequest } from '@navigator_ai/agent-core';
...
// Mock Navigator icon
vi.mock('/assets/navigator-icon.png', () => ({ default: 'navigator-icon.png' }));
...
describe('Execution Page Integration', () => {
  beforeEach(() => {
    ...
    (window as Window & { navigatorApp: typeof mockNavigatorApp }).navigatorApp = mockNavigatorApp;
    // Reset store state
    mockStoreState = {
      currentTask: null,
      loadTaskById: mockLoadTaskById,
  ...
  describe('message display', () => {
    ...
    it('should show thinking indicator when running without tool', () => {
      ...
      expect(
        screen.getByText(/^(Doing|Executing|Running|Handling it|Navigatoring)\.\.\.$/),
      ).toBeInTheDocument();
    });
