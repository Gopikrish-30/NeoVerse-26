import { getNavigatorApp } from './navigator-app';
...
export function initTheme(): void {
  ...
  navigatorApp.getTheme().then((preference) => {
    applyTheme(preference);
  });

  if (navigatorApp.onThemeChange) {
    themeChangeCleanup = navigatorApp.onThemeChange(({ theme }) => {
      applyTheme(theme);
    });
  }
}
