...
export function ExecutionPage() {
  ...
  useEffect(() => {
    navigatorApp.getDebugMode().then(setDebugModeEnabled);
    const unsubscribeDebugMode = navigatorApp.onDebugModeChange?.(({ enabled }) => {
  ...
  useEffect(() => {
    if (id) {
      ...
      navigatorApp.getTodosForTask(id).then((todos) => {
    ...
    const unsubscribeTaskBatch = navigatorApp.onTaskUpdateBatch?.((event) => {
    ...
    const unsubscribeDebugLog = navigatorApp.onDebugLog((log) => {
  ...
  const handleSettingsDialogClose = (open: boolean) => {
    ...
    if (!open) {
      ...
      setSettingsInitialTab('providers');
      speechInput.recheckConfiguration();
    }
  };
  ...
  return (
    <>
      ...
      <div className="h-full flex flex-col bg-background relative">
        {/* Task header */}
        <div className="flex-shrink-0 border-b border-border bg-card/70 backdrop-blur-sm px-6 py-4">
        ...
        {currentTask.status === 'running' && !permissionRequest && (
          <div className="flex-shrink-0 border-t border-border bg-card/70 backdrop-blur-sm px-6 py-4">
