...
export function ZaiProviderForm({
  ...
  const handleConnect = async () => {
    ...
    try {
      const navigatorApp = getNavigatorApp();
      const validation = await navigatorApp.validateApiKeyForProvider('zai', apiKey.trim(), {
        region,
      });
      ...
      await navigatorApp.addApiKey('zai', apiKey.trim());

      // Fetch models dynamically
      let fetchedModels: Array<{ id: string; name: string }> | undefined;
      if (providerConfig?.modelsEndpoint) {
        const fetchResult = await navigatorApp.fetchProviderModels('zai', { zaiRegion: region });
