...
import type { ConnectedProvider, LiteLLMCredentials } from '@navigator_ai/agent-core/common';
import {
  ModelSelector,
  ConnectButton,
  ConnectedControls,
...
export function LiteLLMProviderForm({
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      const result = await navigatorApp.testLiteLLMConnection(serverUrl, trimmedKey);
      if (!result.success) {
      ...
      if (trimmedKey) {
        await navigatorApp.addApiKey('litellm', trimmedKey);
      } else {
        // Remove any previously stored key when connecting without one
        await navigatorApp.removeApiKey('litellm');
      }
