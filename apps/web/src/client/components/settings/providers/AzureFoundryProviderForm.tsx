...
import type { ConnectedProvider, AzureFoundryCredentials } from '@navigator_ai/agent-core/common';
import {
  ModelSelector,
  ConnectButton,
  ConnectedControls,
...
export function AzureFoundryProviderForm({
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      // Validate connection
      const validation = await navigatorApp.testAzureFoundryConnection({
      ...
      // Save credentials
      await navigatorApp.saveAzureFoundryConfig({
        endpoint: endpoint.trim(),
        deploymentName: deploymentName.trim(),
        authType,
        apiKey: authType === 'api-key' ? apiKey.trim() : undefined,
