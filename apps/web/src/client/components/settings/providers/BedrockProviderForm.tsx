...
import { getDefaultModelForProvider } from '@navigator_ai/agent-core/common';
import {
  ModelSelector,
  RegionSelector,
  ConnectButton,
...
export function BedrockProviderForm({
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      const validation = await navigatorApp.validateBedrockCredentials(credentials);
      ...
      // Save credentials
      await navigatorApp.saveBedrockCredentials(credentials);

      // Fetch available models dynamically from AWS
      const credentialsJson = JSON.stringify(credentials);
      const modelsResult = await navigatorApp.fetchBedrockModels(credentialsJson);
