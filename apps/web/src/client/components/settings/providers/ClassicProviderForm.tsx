...
export function ClassicProviderForm({
  ...
  useEffect(() => {
    ...
    navigatorApp.getOpenAiBaseUrl().then(setOpenAiBaseUrl).catch(console.error);
  }, [isOpenAI]);
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      if (isOpenAI) {
        await navigatorApp.setOpenAiBaseUrl(openAiBaseUrl.trim());
      }

      const validation = await navigatorApp.validateApiKeyForProvider(providerId, apiKey.trim());
      ...
      await navigatorApp.addApiKey(providerId as any, apiKey.trim());

      // Fetch models dynamically if provider has a models endpoint
      let fetchedModels: Array<{ id: string; name: string }> | undefined;
      if (providerConfig?.modelsEndpoint) {
        const fetchResult = await navigatorApp.fetchProviderModels(providerId, {
          baseUrl: isOpenAI ? openAiBaseUrl.trim() || undefined : undefined,
        });
  ...
  const handleChatGptSignIn = async () => {
    ...
    try {
      const navigatorApp = getNavigatorApp();
      await navigatorApp.loginOpenAiWithChatGpt();
      const status = await navigatorApp.getOpenAiOauthStatus();
