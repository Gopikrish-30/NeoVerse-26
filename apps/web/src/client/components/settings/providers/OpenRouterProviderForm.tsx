...
import type { ConnectedProvider, OpenRouterCredentials } from '@navigator_ai/agent-core/common';
import { PROVIDER_META } from '@navigator_ai/agent-core/common';
import {
  ModelSelector,
  ConnectButton,
  ConnectedControls,
...
export function OpenRouterProviderForm({
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      // Validate key
      const validation = await navigatorApp.validateApiKeyForProvider('openrouter', apiKey.trim());
      if (!validation.valid) {
      ...
      // Save key
      await navigatorApp.addApiKey('openrouter', apiKey.trim());

      // Fetch models
      const result = await navigatorApp.fetchOpenRouterModels();
      if (!result.success) {
