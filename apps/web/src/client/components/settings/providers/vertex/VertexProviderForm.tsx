...
export function VertexProviderForm({
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      const validation = await navigatorApp.validateVertexCredentials(credentials);
      ...
      // Save credentials
      await navigatorApp.saveVertexCredentials(credentials);

      // Fetch available models
      const credentialsJson = JSON.stringify(credentials);
      const modelsResult = await navigatorApp.fetchVertexModels(credentialsJson);
