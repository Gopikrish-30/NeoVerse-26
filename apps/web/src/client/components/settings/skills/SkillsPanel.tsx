...
import type { Skill } from '@navigator_ai/agent-core/common';
...
export function SkillsPanel({ refreshTrigger }: SkillsPanelProps) {
  ...
  const handleToggle = useCallback(
    async (id: string) => {
      ...
      if (!skill || !window.navigatorApp) return;

      try {
        await window.navigatorApp.setSkillEnabled(id, !skill.isEnabled);
  ...
  const handleDelete = useCallback(
    async (id: string) => {
      ...
      try {
        await window.navigatorApp.deleteSkill(id);
  ...
  const handleEdit = useCallback(async (filePath: string) => {
    if (!window.navigatorApp) return;
    try {
      await window.navigatorApp.openSkillInEditor(filePath);
    } catch (err) {
  ...
  const handleShowInFolder = useCallback(async (filePath: string) => {
    if (!window.navigatorApp) return;
    try {
      await window.navigatorApp.showSkillInFolder(filePath);
    } catch (err) {
  ...
  const handleResync = useCallback(async () => {
    if (!window.navigatorApp || isResyncing) return;
    setIsResyncing(true);
    try {
      // Run resync and minimum delay in parallel so animation is visible
      const [updatedSkills] = await Promise.all([
        window.navigatorApp.resyncSkills(),
