...
import type { Skill, McpConnector } from '@navigator_ai/agent-core/common';
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuTrigger,
...
export function PlusMenu({ onSkillSelect, onOpenSettings, disabled }: PlusMenuProps) {
  ...
  const handleRefresh = async () => {
    ...
    if (!navigatorApp || isRefreshing) return;
    setIsRefreshing(true);
    try {
      const [, updatedSkills] = await Promise.all([
        new Promise((resolve) => setTimeout(resolve, 600)),
        navigatorApp.resyncSkills().then(() => navigatorApp.getEnabledSkills()),
      ]);
  ...
  const handleToggleConnector = useCallback(async (id: string, enabled: boolean) => {
    ...
    try {
      await window.navigatorApp.setConnectorEnabled(id, enabled);
