...
export const useTaskStore = create<TaskState>((set, get) => ({
  ...
  startTask: async (config: TaskConfig) => {
    ...
    try {
      void navigatorApp.logEvent({
        level: 'info',
        message: 'UI start task',
        context: { prompt: config.prompt, taskId: config.taskId },
      });
      const task = await navigatorApp.startTask(config);
  ...
  sendFollowUp: async (message: string) => {
    ...
    try {
      ...
      const task = await navigatorApp.resumeSession(sessionId, message, currentTask.id);
  ...
  cancelTask: async () => {
    ...
    if (currentTask) {
      ...
      await navigatorApp.cancelTask(currentTask.id);
      set((state) => ({
  ...
  interruptTask: async () => {
    ...
    if (currentTask && currentTask.status === 'running') {
      ...
      await navigatorApp.interruptTask(currentTask.id);
    }
  },
  ...
  respondToPermission: async (response: PermissionResponse) => {
    ...
    await navigatorApp.respondToPermission(response);
  ...
  loadTasks: async () => {
    const navigatorApp = getNavigatorApp();
    const tasks = await navigatorApp.listTasks();
    set({ tasks });
  },

  loadTaskById: async (taskId: string) => {
    const navigatorApp = getNavigatorApp();
    const task = await navigatorApp.getTask(taskId);
    set({ currentTask: task, error: task ? null : 'Task not found' });
  },

  deleteTask: async (taskId: string) => {
    const navigatorApp = getNavigatorApp();
    await navigatorApp.deleteTask(taskId);
    set((state) => ({
  ...
  clearHistory: async () => {
    const navigatorApp = getNavigatorApp();
    await navigatorApp.clearTaskHistory();
    set({ tasks: [] });
  },
...
if (typeof window !== 'undefined' && window.navigatorApp) {
  window.navigatorApp.onTaskProgress((progress: unknown) => {
  ...
  window.navigatorApp.onTaskUpdate((event: unknown) => {
  ...
  window.navigatorApp.onTaskSummary?.((data: { taskId: string; summary: string }) => {
    useTaskStore.getState().setTaskSummary(data.taskId, data.summary);
  });

  window.navigatorApp.onTodoUpdate?.((data: { taskId: string; todos: TodoItem[] }) => {
    const state = useTaskStore.getState();
  ...
  window.navigatorApp.onAuthError?.((data: { providerId: string; message: string }) => {
