...
let capturedNavigatorAppAPI: Record<string, unknown> = {};
let capturedNavigatorAppShell: Record<string, unknown> = {};

describe('Preload Script Integration', () => {
  beforeEach(async () => {
    vi.clearAllMocks();
    capturedNavigatorAppAPI = {};
    capturedNavigatorAppShell = {};
    ...
    mockExposeInMainWorld.mockImplementation((name: string, api: unknown) => {
      if (name === 'navigator') {
        capturedNavigatorAppAPI = api as Record<string, unknown>;
      } else if (name === 'navigatorAppShell') {
        capturedNavigatorAppShell = api as Record<string, unknown>;
      }
    });
  ...
  describe('API Exposure', () => {
    it('should expose navigatorApp API via contextBridge', () => {
      expect(mockExposeInMainWorld).toHaveBeenCalledWith('navigator', expect.any(Object));
      expect(capturedNavigatorAppAPI).toBeDefined();
    });

    it('should expose navigatorAppShell info via contextBridge', () => {
      expect(mockExposeInMainWorld).toHaveBeenCalledWith('navigatorAppShell', expect.any(Object));
      expect(capturedNavigatorAppShell).toBeDefined();
    });

    it('should expose shell info with isElectron=true', () => {
      expect(capturedNavigatorAppShell.isElectron).toBe(true);
    });

    it('should expose shell info with platform', () => {
      expect(capturedNavigatorAppShell.platform).toBe(process.platform);
    });

    it('should expose shell info with version matching package.json', () => {
      expect(capturedNavigatorAppShell.version).toBe(pkg.version);
    });
  });
  ...
  describe('IPC Method Invocations', () => {
    describe('App Info', () => {
      it('getVersion should invoke app:version', async () => {
        await (capturedNavigatorAppAPI.getVersion as () => Promise<string>)();
        expect(mockInvoke).toHaveBeenCalledWith('app:version');
      });

      it('getPlatform should invoke app:platform', async () => {
        await (capturedNavigatorAppAPI.getPlatform as () => Promise<string>)();
    ...
    describe('Shell Operations', () => {
      it('openExternal should invoke shell:open-external with URL', async () => {
        ...
        await (capturedNavigatorAppAPI.openExternal as (url: string) => Promise<void>)(url);
    ...
    describe('Task Operations', () => {
      it('startTask should invoke task:start with config', async () => {
        ...
        await (
          capturedNavigatorAppAPI.startTask as (config: { description: string }) => Promise<unknown>
        )(config);
        expect(mockInvoke).toHaveBeenCalledWith('task:start', config);
      });

      it('cancelTask should invoke task:cancel with taskId', async () => {
        await (capturedNavigatorAppAPI.cancelTask as (taskId: string) => Promise<void>)('task_123');
      ...
      it('interruptTask should invoke task:interrupt with taskId', async () => {
        await (capturedNavigatorAppAPI.interruptTask as (taskId: string) => Promise<void>)(
          'task_123',
        );
    ...
    describe('Permission Operations', () => {
      it('respondToPermission should invoke permission:respond', async () => {
        ...
        await (
          capturedNavigatorAppAPI.respondToPermission as (r: {
            taskId: string;
            allowed: boolean;
          }) => Promise<void>
        )(response);
    ...
    describe('Session Operations', () => {
      it('resumeSession should invoke session:resume', async () => {
        await (
          capturedNavigatorAppAPI.resumeSession as (
            s: string,
            p: string,
            t?: string,
          ) => Promise<unknown>
    ...
    describe('Settings Operations', () => {
      it('getDebugMode should invoke settings:debug-mode', async () => {
        await (capturedNavigatorAppAPI.getDebugMode as () => Promise<boolean>)();
        expect(mockInvoke).toHaveBeenCalledWith('settings:debug-mode');
      });

      it('setDebugMode should invoke settings:set-debug-mode', async () => {
        await (capturedNavigatorAppAPI.setDebugMode as (enabled: boolean) => Promise<void>)(true);
      ...
      it('getAppSettings should invoke settings:app-settings', async () => {
        await (capturedNavigatorAppAPI.getAppSettings as () => Promise<unknown>)();
    ...
    describe('Onboarding Operations', () => {
      it('getOnboardingComplete should invoke onboarding:complete', async () => {
        await (capturedNavigatorAppAPI.getOnboardingComplete as () => Promise<boolean>)();
    ...
    describe('Model Operations', () => {
      it('getSelectedModel should invoke model:get', async () => {
        await (capturedNavigatorAppAPI.getSelectedModel as () => Promise<unknown>)();
      ...
      it('setSelectedModel should invoke model:set', async () => {
        ...
        await (
          capturedNavigatorAppAPI.setSelectedModel as (m: {
            provider: string;
            model: string;
          }) => Promise<void>
        )(model);
    ...
    describe('Logging Operations', () => {
      it('logEvent should invoke log:event', async () => {
        ...
        await (capturedNavigatorAppAPI.logEvent as (p: unknown) => Promise<unknown>)(payload);
    ...
    describe('Skills Operations', () => {
      it('getUserSkillsPath should invoke skills:get-user-skills-path', async () => {
        await (capturedNavigatorAppAPI.getUserSkillsPath as () => Promise<string>)();
  ...
  describe('Event Subscriptions', () => {
    it('onTaskUpdate should subscribe to task:update', () => {
      ...
      (capturedNavigatorAppAPI.onTaskUpdate as (cb: (e: unknown) => void) => () => void)(callback);
    ...
    it('onTaskUpdate should return unsubscribe function', () => {
      const callback = vi.fn();
      const unsubscribe = (
        capturedNavigatorAppAPI.onTaskUpdate as (cb: (e: unknown) => void) => () => void
      )(callback);
      unsubscribe();
    ...
    it('onTaskUpdateBatch should subscribe to task:update:batch', () => {
      const callback = vi.fn();
      (capturedNavigatorAppAPI.onTaskUpdateBatch as (cb: (e: unknown) => void) => () => void)(
        callback,
      );
    ...
    it('onPermissionRequest should subscribe to permission:request', () => {
      const callback = vi.fn();
      (capturedNavigatorAppAPI.onPermissionRequest as (cb: (e: unknown) => void) => () => void)(
        callback,
      );
    ...
    it('onDebugLog should subscribe to debug:log', () => {
      const callback = vi.fn();
      (capturedNavigatorAppAPI.onDebugLog as (cb: (e: unknown) => void) => () => void)(callback);
    ...
    it('onTaskStatusChange should subscribe to task:status-change', () => {
      const callback = vi.fn();
      (capturedNavigatorAppAPI.onTaskStatusChange as (cb: (e: unknown) => void) => () => void)(
        callback,
      );
