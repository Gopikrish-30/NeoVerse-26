...
// Note: PERMISSION_API_PORT and QUESTION_API_PORT are now imported from @navigator/shared
// by config-generator.ts, so no mock needed here

// Mock @navigator_ai/agent-core (uses SQLite which requires native module)
// Note: generateConfig mock creates real files in temp directory for integration testing
vi.mock('@navigator_ai/agent-core', async () => {
...
Use AskUserQuestion tool for user interaction.`,
        ...
        return {
          configPath,
          systemPrompt: config.agent.navigator.prompt,
    ...
    NAVIGATOR_AGENT_NAME: 'navigator',
...
describe('OpenCode Config Generator Integration', () => {
  ...
  describe('generateOpenCodeConfig()', () => {
    ...
    it('should write config file with correct structure', async () => {
      ...
      expect(config.default_agent).toBe('navigator');
    ...
    it('should include navigatorApp agent configuration', async () => {
      // Act
      const { generateOpenCodeConfig } = await import('@main/opencode/config-generator');
      const configPath = await generateOpenCodeConfig();
      ...
      const agent = config.agent['navigator'];
    ...
    it('should include platform-specific environment instructions', async () => {
      ...
      // Assert
      const config = JSON.parse(fs.readFileSync(configPath, 'utf-8'));
      const prompt = config.agent['navigator'].prompt;
    ...
    it('should include enabled connected connectors with access tokens', async () => {
      ...
      const { generateConfig } = await import('@navigator_ai/agent-core');
  ...
  describe('NAVIGATOR_AGENT_NAME Export', () => {
    it('should export the agent name constant', async () => {
      // Act
      const { NAVIGATOR_AGENT_NAME } = await import('@main/opencode/config-generator');

      // Assert
      expect(NAVIGATOR_AGENT_NAME).toBe('navigator');
    });
  });
