...
import type { Task, TaskMessage, TaskStatus } from '@navigator_ai/agent-core';
...
vi.mock('@navigator_ai/agent-core', () => ({
...
describe('taskHistory Integration', () => {
  ...
  describe('saveTask and getTask', () => {
    it('should save and retrieve a task by ID', async () => {
      // Arrange
      const { saveTask, getTask } = await import('@navigator_ai/agent-core');
    ...
    it('should return null for non-existent task', async () => {
      // Arrange
      const { getTask } = await import('@navigator_ai/agent-core');
  ...
  describe('getTasks', () => {
    it('should return empty array when no tasks exist', async () => {
      // Arrange
      const { getTasks } = await import('@navigator_ai/agent-core');
    ...
    it('should return all saved tasks', async () => {
      // Arrange
      const { saveTask, getTasks } = await import('@navigator_ai/agent-core');
  ...
  describe('updateTaskStatus', () => {
    it('should update task status', async () => {
      // Arrange
      const { saveTask, getTask, updateTaskStatus } = await import('@navigator_ai/agent-core');
  ...
  describe('updateTaskSessionId', () => {
    it('should update session ID for existing task', async () => {
      // Arrange
      const { saveTask, getTask, updateTaskSessionId } = await import('@navigator_ai/agent-core');
      saveTask(createMockTask('task-1'));
  ...
  describe('updateTaskSummary', () => {
    it('should update task summary', async () => {
      // Arrange
      const { saveTask, getTask, updateTaskSummary } = await import('@navigator_ai/agent-core');
      saveTask(createMockTask('task-1'));
  ...
  describe('addTaskMessage', () => {
    it('should add message to existing task', async () => {
      // Arrange
      const { saveTask, getTask, addTaskMessage } = await import('@navigator_ai/agent-core');
      saveTask(createMockTask('task-1'));
  ...
  describe('deleteTask', () => {
    it('should delete task by ID', async () => {
      // Arrange
      const { saveTask, getTask, deleteTask } = await import('@navigator_ai/agent-core');
      saveTask(createMockTask('task-1'));
    ...
    it('should not throw when deleting non-existent task', async () => {
      // Arrange
      const { deleteTask } = await import('@navigator_ai/agent-core');
  ...
  describe('clearHistory', () => {
    it('should remove all tasks', async () => {
      // Arrange
      const { saveTask, getTasks, clearHistory } = await import('@navigator_ai/agent-core');
      saveTask(createMockTask('task-1'));
  ...
  describe('flushPendingTasks', () => {
    it('should be a no-op for SQLite (writes are immediate)', async () => {
      // Arrange
      const { flushPendingTasks } = await import('@navigator_ai/agent-core');
