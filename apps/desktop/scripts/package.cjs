...
const navigatorAiPath = path.join(nodeModulesPath, '@navigator_ai');
...
try {
  ...
  for (const pkg of workspacePackages) {
    const pkgPath = path.join(navigatorAiPath, pkg);
  ...
  // Remove empty @navigator_ai directory if it exists
  if (Object.keys(symlinkTargets).length > 0) {
    try {
      fs.rmdirSync(navigatorAiPath);
    } catch {
  ...
  if (packagesToRestore.length > 0) {
    ...
    // Recreate @navigator_ai directory if needed
    if (!fs.existsSync(navigatorAiPath)) {
      fs.mkdirSync(navigatorAiPath, { recursive: true });
    }
