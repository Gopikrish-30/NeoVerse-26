...
1. **Use the base path from the task prompt** when present.
   - If no path is provided, detect the OS and use:
     - macOS: `~/Library/Application Support/Navigator/skills/`
     - Windows: `%APPDATA%\Navigator\skills\`
     - Linux: `~/.config/Navigator/skills/`
...
5. **The skill is automatically detected** - Navigator scans this directory on startup and syncs new skills to its database. The skill will appear in Settings > Skills as a "Custom" skill.
