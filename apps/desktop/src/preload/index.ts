...
import type { ProviderType, Skill, TodoItem, McpConnector } from '@navigator_ai/agent-core';

// Expose the navigatorApp API to the renderer
const navigatorAppAPI = {
  // App info
  ...
  telegramRemoveToken: (): Promise<void> => ipcRenderer.invoke('telegram:remove-token'),
  ...
  }> => ipcRenderer.invoke('telegram:get-status'),
  telegramGeneratePairing: (): Promise<{
    pin: string;
    qrCodeDataUrl: string;
    botLink: string;
  }> => ipcRenderer.invoke('telegram:generate-pairing'),
  telegramUnpair: (): Promise<void> => ipcRenderer.invoke('telegram:unpair'),
  telegramHasToken: (): Promise<boolean> => ipcRenderer.invoke('telegram:has-token'),
  telegramApprovePairing: (requestId: string): Promise<void> =>
    ipcRenderer.invoke('telegram:approve-pairing', requestId),
  telegramDenyPairing: (requestId: string): Promise<void> =>
    ipcRenderer.invoke('telegram:deny-pairing', requestId),
  onTelegramPairingRequest: (
    ...
    ipcRenderer.on('telegram:pairing-request', listener);
    return () => {
      ipcRenderer.removeListener('telegram:pairing-request', listener);
    };
  },
};

// Expose the API to the renderer
contextBridge.exposeInMainWorld('navigatorApp', navigatorAppAPI);
...
contextBridge.exposeInMainWorld('navigatorAppShell', {
...
// Type declarations
export type NavigatorAppAPI = typeof navigatorAppAPI;
