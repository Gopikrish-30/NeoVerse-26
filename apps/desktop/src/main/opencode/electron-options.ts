...
import type { TaskManagerOptions, TaskCallbacks } from '@navigator_ai/agent-core';
import type { TaskConfig } from '@navigator_ai/agent-core';
import { DEV_BROWSER_PORT } from '@navigator_ai/agent-core';
import {
  getAzureEntraToken,
...
import { getModelDisplayName } from '@navigator_ai/agent-core';
import type {
  ...
  if (!bundledNode) {
    throw new Error(
      '[OpenCode CLI] Bundled Node.js path is missing. ' +
        'Run "pnpm -F @navigator/desktop download:nodejs" and rebuild before launching.',
    );
  }
