...
import { createTaskId, createMessageId } from '@navigator_ai/agent-core';
import {
  storeApiKey,
  getApiKey,
  deleteApiKey,
...
import { getOpenAiOauthStatus } from '@navigator_ai/agent-core';
...
export function registerIPCHandlers(): void {
  ...
  handle('task:cancel', async (_event: IpcMainInvokeEvent, taskId?: string) => {
    ...
    // Deny any pending permission/question requests so MCP tools unblock
    denyPendingRequestsForTask(taskId);
    ...
    async (
      ...
      if (STANDARD_VALIDATION_PROVIDERS.has(provider)) {
        ...
        const result = await validateApiKey(
          provider as import('@navigator_ai/agent-core').ProviderType,
          sanitizedKey,
          {
            timeout: API_KEY_VALIDATION_TIMEOUT_MS,
            baseUrl:
  ...
  handle(
    ...
    async (
      ...
      if (providerId === 'zai' && options?.zaiRegion) {
        const region = options.zaiRegion as import('@navigator_ai/agent-core').ZaiRegion;
  ...
  handle('logs:export', async (event: IpcMainInvokeEvent) => {
    ...
    const defaultFilename = `navigator-logs-${timestamp}.txt`;
    ...
    try {
      if (fs.existsSync(logPath)) {
        ...
        const header = `Navigator Application Logs\nExported: ${new Date().toISOString()}\nLog Directory: ${logDir}\n\nNo logs recorded yet.\n`;
  ...
  handle('connectors:start-oauth', async (_event, connectorId: string) => {
    ...
    const authUrl = buildAuthorizationUrl({
      ...
      redirectUri: 'navigator://callback/mcp',
  ...
  handle('telegram:set-token', async (_event: IpcMainInvokeEvent, token: string) => {
    ...
    return telegramService.setBotToken(token.trim());
  });

  handle('telegram:remove-token', async (_event: IpcMainInvokeEvent) => {
    const telegramService = getTelegramService();
    await telegramService.removeBotToken();
  });

  handle('telegram:set-enabled', async (_event: IpcMainInvokeEvent, enabled: boolean) => {
    if (typeof enabled !== 'boolean') {
      throw new Error('Invalid enabled flag');
    }
    const telegramService = getTelegramService();
    await telegramService.setEnabled(enabled);
  });

  handle('telegram:get-status', async (_event: IpcMainInvokeEvent) => {
    const telegramService = getTelegramService();
    return telegramService.getStatus();
  });

  handle('telegram:generate-pairing', async (_event: IpcMainInvokeEvent) => {
    const telegramService = getTelegramService();
    return telegramService.generatePairingPinAndQR();
  });

  handle('telegram:unpair', async (_event: IpcMainInvokeEvent) => {
    const telegramService = getTelegramService();
    telegramService.unpair();
  });

  handle('telegram:has-token', async (_event: IpcMainInvokeEvent) => {
    const telegramService = getTelegramService();
    return telegramService.hasToken();
  });

  handle('telegram:approve-pairing', async (_event: IpcMainInvokeEvent, requestId: string) => {
    if (typeof requestId !== 'string' || !requestId.trim()) {
      throw new Error('Invalid request ID');
    }
    const telegramService = getTelegramService();
    await telegramService.approvePairing(requestId.trim());
  });

  handle('telegram:deny-pairing', async (_event: IpcMainInvokeEvent, requestId: string) => {
    if (typeof requestId !== 'string' || !requestId.trim()) {
      throw new Error('Invalid request ID');
    }
    const telegramService = getTelegramService();
    await telegramService.denyPairing(requestId.trim());
  });
  handle(
    'file:save-image',
    async (
