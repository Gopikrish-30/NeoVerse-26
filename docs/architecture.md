...
packages/agent-core/     # Core business logic, shared types, MCP tools (published as @navigator_ai/agent-core)
```

## Package Dependency Graph

```
@navigator_ai/agent-core (business logic, types, constants, storage, MCP tools)
        ↑
@navigator/desktop (Electron app)
```
...
- `index.ts` - Electron bootstrap, single-instance enforcement, `navigator://` protocol handler
...
- Exposes `window.navigatorApp` API via `contextBridge`
- Provides `window.navigatorAppShell` for shell metadata
...
- `lib/navigatorApp.ts` - Typed wrapper for the IPC API
...
App data is stored in SQLite (`navigator.db` in production, `navigator-dev.db` in development) located in the user data directory.
...
import { getNpxPath, getBundledNodePaths } from '@navigator_ai/agent-core/utils';
